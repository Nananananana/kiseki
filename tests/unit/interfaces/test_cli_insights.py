"""The insights command surfaces the findings, with their grounding."""
...
class TestInsightsCommand:
    ...
    def test_reports_the_findings_once_the_history_has_grown(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
    ...
    def test_json_carries_the_contract(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        _seed(tmp_path, _profile(0, "onsen"), _profile(20, "museum"))
        assert _run(tmp_path, "insights", "--json") == EXIT_OK
        payload = json.loads(capsys.readouterr().out)
        first = payload["insights"][0]
        assert first["kind"] == "new"
        assert first["novelty"] == 1.0
        assert first["evidence"] == ["caption:aa"]
