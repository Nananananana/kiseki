"""/discover answers honestly on an empty history."""
...
def test_discover_reports_not_enough_history():
    server = make_server(_pipeline, lambda: None, host="127.0.0.1", port=0)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        with urllib.request.urlopen(
            f"http://127.0.0.1:{server.server_address[1]}/discover"
        ) as response:
            payload = json.loads(response.read().decode("utf-8"))
        assert payload["discoveries"] is None
    finally:
        server.shutdown()
        server.server_close()
