...
| `ask` | Answer a question from the readings, with evidence, saying which derivation it read the question as being about; `--json` blurred unless `--raw` |
...
| `cost` | What the model work still to do will take, measured on this machine |
...
| `errors` | Every named way a command here can stop, its exit code, and whether to retry |
